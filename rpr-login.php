	...
	class RPR_Login {
		...
		public /*.void.*/ function rpr_register_form() {
			...
			<table border="0" cellpadding="0" cellspacing="0" width="80%">
				...
				<tr style="height: 2.5em;">
					<td style="vertical-align:middle;" width="150px">E-mail:</td>
					<td><input name="user_email" id="user_email" class="input" value="" size="25" type="email" value=""/></td>
				</tr>

				<tr style="height: 2.5em;">
					<td style="vertical-align:middle;" width="150px">Confirm E-mail:</td>
					<td><input type="text" autocomplete="off" name="user_email2" id="user_email2" class="input" value=""/></td>
				</tr>

				<tr style="height: 2.5em;">
					<td style="vertical-align:middle;" width="150px">Password:</td>
					<td><input type="password" autocomplete="off" name="pass1" id="pass1"/></td>
				</tr>

				<tr style="height: 2.5em;">
					<td style="vertical-align:middle;" width="150px">Confirm Password:</td>
					<td><input type="password" autocomplete="off" name="pass2" id="pass2"/></td>
				</tr>

				<tr style="height: 2.5em;">
					<td style="vertical-align:middle;" width="150px">First Name:</td>
					<td><input type="text" name="first_name" id="first_name" class="input" value=""/></td>
				</tr>

				<tr style="height: 2.5em;">
					<td style="vertical-align:middle;" width="150px">Last Name:</td>
					<td><input type="text" name="last_name" id="last_name" class="input" value=""/></td>
				</tr>

				<tr style="height: 2.5em;">
					<td style="vertical-align:middle;" width="150px">Organization:</td>
					<td><input name="organization" id="organization" class="input" value="" type="text" value=""/></td>>
				</tr>

				<tr style="height: 2.5em;">
					<td style="vertical-align:middle;" width="150px">Country:</td>
					<td><input name="country" id="country" class="input" value="" type="text" value=""/></td>>
				</tr>
				<!-- very important to use 'security' as ID. Using other ID and it wont submit correctly -->
				<tr style="height: 2.5em;">
					<td><input type="hidden" id="security" name="security" class="input"></td>
				</tr>
			...
			<script>
				var parent= document.getElementById("registerform"); 
		...
		public /*.object.*/ function rpr_registration_errors( /*.object.*/ $errors, /*.string.*/ $sanitized_user_login, /*.string.*/ $user_email ) {
			...
			if (isset($_POST['security'])) {
				$captcha = $_POST['security'];
			} else {
				$captcha = false;
			}
			
			if (!$captcha) {
				$errors->add( 'invalid_security', '<strong>' . __( 'ERROR', 'register-plus-redux' ) . '</strong>:&nbsp;' . __( 'No recaptcha.', 'register-plus-redux' ) );
				//$errors->add( 'invalid_security', '<strong>' . __( 'ERROR', 'register-plus-redux' ) . '</strong>:&nbsp;' . __( 'Google captcha3 not detected.', 'register-plus-redux' ) );
			...
			else
			{
				...
				// $errors->add( 'invalid_security', '<strong>' . __( 'ERROR', 'register-plus-redux' ) . '</strong>:&nbsp;' . __( 'We see recaptcha.', 'register-plus-redux' ) );
				...
				if ($response->success === false) {
					$errors->add( 'invalid_security', '<strong>' . __( 'ERROR', 'register-plus-redux' ) . '</strong>:&nbsp;' . __( 'The Google captcha3 system has warned that your request might be from a malicious system. Please try again after a few minutes or contact us to help you, weekdays 9 AM to 5 PM PST. Email: support at neyrinck dot com. Facebook @neyrinckaudio.', 'register-plus-redux' ) );
				}
				else {
					...
					if ($response->success==true && $response->score <= 0.3) {
						$msg = "Google recaptcha3 believes this is spam. score = $score. Please try again after a few minutes or contact us to help you, weekdays 9 AM to 5 PM PST. Email: support at neyrinck dot com. Facebook @neyrinckaudio.";
						$errors->add( 'invalid_security', '<strong>' . __( 'ERROR', 'register-plus-redux' ) . '</strong>:&nbsp;' . $msg );
					}
				}
			...
			/*	
				$sImg = new Securimage();
				$valid = $sImg->check($_POST['security']);
				if($valid != TRUE) {
					$errors->add( 'invalid_security', '<strong>' . __( 'ERROR', 'register-plus-redux' ) . '</strong>:&nbsp;' . __( 'Please enter a valid security code.', 'register-plus-redux' ) );
			...
			if ( isset( $_GET['verification_code'] ) && '1' === $register_plus_redux->rpr_get_option( 'verify_user_email' ) ) {
				...
				if ( !is_wp_error( $user_id ) ) {
					if ( '1' !== $register_plus_redux->rpr_get_option( 'verify_user_admin' ) ) {
						...
						if ( '1' === $register_plus_redux->rpr_get_option( 'user_set_password' ) ) {
							$errors->add( 'account_verified', sprintf( __( 'Thank you %s, your account has been verified, please login with the email and password you specified during registration.', 'register-plus-redux' ), $user->first_name ), 'message' );
						}
						else {
		...
		public /*.void.*/ function rpr_login_head() {
			...
			<script>
				...
				// response is promise with passed token
					grecaptcha.execute('6LfduNcUAAAAAMCW0-tF_IDCz2gew_xTS1wYW2Mh', {action:'register'})
							.then(function(token) {
						// add token value to form
						document.getElementById('security').value = token;
					});
				});
			</script>";
			global $register_plus_redux;
